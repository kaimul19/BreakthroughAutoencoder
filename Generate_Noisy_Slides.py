...
def get_cached_frame(
    number_of_frequency_bins: int, total_time_channels: int
) -> stg.Frame:
    """
...
def generate_single_background_cadence(
    number_of_observations: int, number_of_time_bins: int, number_of_frequency_bins: int
) -> np.ndarray:
    """
    ...
    frame = get_cached_frame(
        number_of_frequency_bins, number_of_observations * number_of_time_bins
    )
...
def chunk_and_generate_background(
    memory_map_file_path: str,
    ...
    memory_map_data = np.memmap(
        memory_map_file_path, dtype="float32", mode="w+", shape=data_dimensions
    )
    ...
    chunk_bar = tqdm(
        range(number_of_chunks), desc="Processing chunks", unit="chunk", position=0
    )
    ...
    for sample_to_plot in range(0, 10, 2):
        ...
        for obs_idx in range(6):
            img = axes_array[obs_idx].imshow(
                background_data[sample_to_plot, obs_idx], aspect="auto"
            )
            axes_array[obs_idx].set_ylabel(f"cadence {obs_idx}", fontsize=8)
